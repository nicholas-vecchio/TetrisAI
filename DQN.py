...
TARGET_UPDATE = 100  # Update the target network every 100 experiences
...
class DQNAgent:
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        self.memory.push(torch.tensor(state["grid"], device=device), action_index, torch.tensor([reward], device=device), torch.tensor(next_state["grid"], device=device), done)
    ...
    def learn(self, transitions):
        ...
        non_final_next_states = non_final_next_states.view(-1, 20 * 10)
