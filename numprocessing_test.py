...
def main(agent, shared_experience):
    ...
    local_batch = deque(maxlen=BATCH_SIZE)  # Local batch for experiences
...
def run_with_parallelism(parallelism):
    manager = Manager()
    ...
    print(f"Total Execution Time with {parallelism} processes: {total_execution_time} seconds")
...
if __name__ == "__main__":
    ...
    for parallelism in range(min_parallelism, max_parallelism + 1):
        print(f"Running with {parallelism} processes...")
        execution_time = run_with_parallelism(parallelism)
        execution_times.append((parallelism, execution_time))

    # Plot the results
    parallelism_values, execution_time_values = zip(*execution_times)
    plt.plot(parallelism_values, execution_time_values, marker='o')
    plt.xlabel('Number of Processes')
    plt.ylabel('Total Execution Time (seconds)')
    plt.title('Execution Time vs. Number of Processes')
    plt.grid(True)
    plt.savefig('execution_time_vs_processes.png')
    plt.show()